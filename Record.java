...
public class Record {
	...
	public Record(DataSet parentSet, int recordNumber, double[] values){
		this.parentDataSet = parentSet;
		this.recordNumber = recordNumber;
		this.values = new Double[values.length-1];
		for(int i = 0; i < values.length-1; i++)
			this.values[i] = values[i];
		classification = (int)values[values.length-1];
	}
	...
	public String toString(){
		...
		stringBuilder.append("Record #" + recordNumber + " :: Y: " + classification);
